...
// ==========================================
// 9. INIT DB (CREATE EMPTY COLLECTIONS)
// ==========================================
exports.initDb = async (req, res) => {
    try {
        // Create dummy docs
        const log = await AuditLog.create({ adminId: req.user ? req.user.id : null, action: 'SYSTEM_INIT', targetType: 'System', targetId: '123' });
        const inc = await Incident.create({ reportedBy: null, targetUser: null, type: 'Safety', description: 'Init' });
        const em = await Emergency.create({ patientId: null, type: 'Medical', location: { type: 'Point', coordinates: [0,0] } });
        
        // Delete them immediately so they don't pollute the DB
        await AuditLog.findByIdAndDelete(log._id);
        await Incident.findByIdAndDelete(inc._id);
